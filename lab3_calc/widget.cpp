...
void Widget::on_resbutton_clicked() //вычисление результата
{
    double op1 = 0, // операнд 1
            op2  = 0, //операнд 2
            res = 0; //результат

    bool ok = false; //флаг успешности

    QString str; //строка для преобразования

    if (ui->sum->isChecked())//блок суммы
    {
        str.clear(); //очищаю строк от мусора
        ...
        if (ok)// if operand 1 == true
        {
            ...
            {
                res = op1 + op2;
                if (!qIsInf(res)) //Если "res" не равно бесконечности  и не равен числу NaN
                {
                    if (!qIsNaN(res))
                    {
                        str.setNum(res);
                    ...
                    {
                        ui->errorlabel->setText("Результат неопределен");
                ...
                {
                    ui->errorlabel->setText("Результат вышел за пределы");
                    ui->errorlabel->setVisible(true);
                    ui->resultLabel->setVisible(false);
                }
            }
        ...
        if (ok)
        {
            ...
            if (ok)
            {
                ...
                else
                {
                    if (op2 == 0)
                    {
                        ui->errorlabel->setText("Деление на 0");
                        ui->errorlabel->setVisible(true);
                        ui->resultLabel->setVisible(false);
                    }
                    else
